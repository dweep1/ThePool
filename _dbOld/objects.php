...
class week extends event{
    ...
    public static function getNextLock($offset){
        ...
        return self::getTimezoneLockTime($game->getLockTime(), $offset);
    ...
    public function getStructured($user_id = null, $noIndex = false){
        ...
        if(!is_bool($this->games)){
            foreach($this->games as $key => $val){
				...
				$this->games[$key]->lockTime = $this->games[$key]->getLockTime();
				$unDato = new DateTime("now");
				$this->games[$key]->currentTime = $unDato->format('Y-m-d H:i:s');
